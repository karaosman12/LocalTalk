...
io2.on('connection',function(socket){
    ...
    // MEDYA & DOSYA
    socket.on('send-media',function(data){if(!users[socket.id]||!flood()||isMuted(uid).muted)return;var r=users[socket.id].currentRoom;if(!r||!rooms[r]||!data||!data.file)return;try{var b64=data.file.replace(/^data:[^;]+;base64,/,''),buf=Buffer.from(b64,'base64'),sz=buf.length/1048576;if(sz>LIM.fileSize){socket.emit('msg-error','Max '+LIM.fileSize+'MB!');return}var mt3='image',ext='jpg',ft=sanitize(data.fileType||'');if(ft.indexOf('png')!==-1)ext='png';else if(ft.indexOf('gif')!==-1)ext='gif';else if(ft.indexOf('webp')!==-1)ext='webp';else if(ft.indexOf('mp4')!==-1){ext='mp4';mt3='video'}else if(ft.indexOf('webm')!==-1){ext='webm';mt3='video'}else if(ft.indexOf('mov')!==-1){ext='mov';mt3='video'}// ✅ YENİ
var fn=mt3+'_'+Date.now()+'_'+rand()+'.'+ext;
fs.writeFile(path.join(MD,fn),buf,function(writeErr){
    if(writeErr){socket.emit('msg-error','Kayıt hatası!');return}
    var msg={id:Date.now()+'_'+rand(),type:mt3,username:users[socket.id].name,role:getRole(users[socket.id].name),mediaFile:fn,mediaType:ft,fileSize:sz.toFixed(1)+'MB',time:getTime(),date:new Date().toLocaleDateString('tr-TR'),readBy:[users[socket.id].name],uid:uid,ip:cIP,replyTo:data.replyTo||null};
    rooms[r].messages.push(msg);hasUnsaved=true;logIDMsg(uid,users[socket.id].name,r,msg);io2.to('room_'+r).emit('new-message',msg);
});
return;var msg={id:Date.now()+'_'+rand(),type:mt3,username:users[socket.id].name,role:getRole(users[socket.id].name),mediaFile:fn,mediaType:ft,fileSize:sz.toFixed(1)+'MB',time:getTime(),date:new Date().toLocaleDateString('tr-TR'),readBy:[users[socket.id].name],uid:uid,ip:cIP,replyTo:data.replyTo||null};rooms[r].messages.push(msg);hasUnsaved=true;logIDMsg(uid,users[socket.id].name,r,msg);io2.to('room_'+r).emit('new-message',msg)}catch(e){socket.emit('msg-error','Hata!')}});
